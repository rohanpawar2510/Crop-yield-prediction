...
const colorMap = {
  emerald: { bg: 'bg-emerald-500/10', text: 'text-emerald-600 dark:text-emerald-400', border: 'border-emerald-500/20', tag: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' },
  blue:    { bg: 'bg-blue-500/10',    text: 'text-blue-600 dark:text-blue-400',       border: 'border-blue-500/20',    tag: 'bg-blue-500/10 text-blue-600 dark:text-blue-400' },
  purple:  { bg: 'bg-purple-500/10',  text: 'text-purple-600 dark:text-purple-400',   border: 'border-purple-500/20',  tag: 'bg-purple-500/10 text-purple-600 dark:text-purple-400' },
  red:     { bg: 'bg-red-500/10',     text: 'text-red-600 dark:text-red-400',         border: 'border-red-500/20',     tag: 'bg-red-500/10 text-red-600 dark:text-red-400' },
  amber:   { bg: 'bg-amber-500/10',   text: 'text-amber-600 dark:text-amber-400',     border: 'border-amber-500/20',   tag: 'bg-amber-500/10 text-amber-600 dark:text-amber-400' },
  teal:    { bg: 'bg-teal-500/10',    text: 'text-teal-600 dark:text-teal-400',       border: 'border-teal-500/20',    tag: 'bg-teal-500/10 text-teal-600 dark:text-teal-400' },
};
...
function LandingNavbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled
        ? 'bg-white/90 dark:bg-gray-950/90 backdrop-blur-xl border-b border-gray-200 dark:border-white/5 shadow-lg'
        : 'bg-transparent'
    }`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 h-16">

        {/* Logo */}
        <Link to="/" className="flex items-center gap-2.5 font-bold text-lg text-gray-900 dark:text-white">
          <div className="p-1.5 rounded-lg bg-emerald-500 text-white">
            <Sprout size={20} />
          </div>
          <span className="tracking-tight">Smart<span className="text-emerald-500 dark:text-emerald-400">Agri</span></span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          <button onClick={toggleTheme}
            className="p-2 rounded-xl text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 transition-all duration-200">
            {isDark ? <Sun size={18} /> : <Moon size={18} />}
          </button>
          {loggedIn ? (
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-500 dark:text-gray-400">
                Hi, <span className="text-gray-900 dark:text-white font-medium">{user?.name?.split(' ')[0]}</span>
              </span>
              <Link to="/dashboard">
            ...
            <div className="flex items-center gap-2">
              <Link to="/login">
                <button className="text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white px-4 py-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 transition">
                  Sign In
                </button>
              </Link>
              <Link to="/register">
        ...
        <div className="flex md:hidden items-center gap-2">
          <button onClick={toggleTheme} className="p-2 rounded-xl text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
            {isDark ? <Sun size={18} /> : <Moon size={18} />}
          </button>
          <button onClick={() => setMenuOpen(!menuOpen)} className="p-2 rounded-xl text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden bg-white/95 dark:bg-gray-950/95 backdrop-blur-xl border-t border-gray-200 dark:border-white/5 px-6 py-5 flex flex-col gap-4">
          {navLinks.map(({ label, href }) => (
            ...
            {loggedIn ? (
              ...
              <>
                <Link to="/login" onClick={() => setMenuOpen(false)}>
                  <button className="w-full border border-gray-300 dark:border-white/10 text-gray-700 dark:text-gray-300 py-2.5 rounded-xl text-sm">
                    Sign In
                  </button>
                </Link>
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-white overflow-x-hidden transition-colors duration-300">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center pt-20">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full px-4 py-1.5 text-sm text-emerald-600 dark:text-emerald-400 font-medium mb-8">
          ...
          <motion.h1
            ...
            <span className="text-gray-900 dark:text-white">Farm Smarter</span>
            <br />
            <span className="bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-400 dark:from-emerald-400 dark:via-teal-400 dark:to-emerald-300 bg-clip-text text-transparent">
              with AI Precision
            </span>
          </motion.h1>
          ...
          <motion.div
            ...
            {loggedIn ? (
              ...
              <>
                ...
                <Link to="/login">
                  <button className="flex items-center gap-2 border border-gray-300 dark:border-white/10 hover:border-gray-400 dark:hover:border-white/20 hover:bg-gray-50 dark:hover:bg-white/5 text-gray-700 dark:text-gray-300 font-semibold px-8 py-4 rounded-2xl transition text-base">
                    Sign In
                  </button>
                </Link>
              </>
          ...
          <motion.div
            ...
            className="flex flex-wrap justify-center gap-6 mt-10 text-xs text-gray-500 dark:text-gray-500">
            {[
              '✅ Free to use',
              '🔒 JWT secured',
          ...
          <motion.div
            ...
            className="mt-16 flex flex-col items-center gap-1 text-gray-400 dark:text-gray-600 cursor-pointer"
      ...
      {/* ── Stats ── */}
      <section id="stats" className="py-20 border-t border-gray-200 dark:border-white/5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <motion.div key={label}
                ...
                className="text-center p-6 rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-emerald-500/40 dark:hover:border-emerald-500/30 transition-all duration-300">
                <p className="text-3xl font-black text-emerald-600 dark:text-emerald-400 mb-1">{value}</p>
      ...
      <section id="features" className="py-24">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div
            ...
            <p className="text-emerald-600 dark:text-emerald-400 text-sm font-semibold tracking-widest uppercase mb-3">What We Offer</p>
            ...
            {features.map(({ icon: Icon, title, desc, color, tag }, i) => {
              ...
              return (
                <motion.div key={title}
                  ...
                  className={`p-6 rounded-2xl bg-gray-50 dark:bg-gray-900 border ${c.border} hover:bg-gray-100 dark:hover:bg-gray-800/60 transition-all duration-300`}>
      ...
      {/* ── How It Works ── */}
      <section id="how" className="py-24 border-t border-gray-200 dark:border-white/5">
        <div className="max-w-5xl mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }} className="text-center mb-16">
            <p className="text-emerald-600 dark:text-emerald-400 text-sm font-semibold tracking-widest uppercase mb-3">Simple Process</p>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-4 text-gray-900 dark:text-white">From Soil to Insight</h2>
      ...
      <section className="py-24">
        <div className="max-w-4xl mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="relative rounded-3xl overflow-hidden border border-emerald-500/20 bg-gradient-to-br from-emerald-50 via-gray-50 to-teal-50 dark:from-emerald-950/80 dark:via-gray-900 dark:to-teal-950/50 p-12 md:p-16 text-center">
            ...
            <div className="relative z-10">
              ...
              {loggedIn ? (
                ...
                <div className="flex flex-wrap gap-4 justify-center">
                  ...
                  <Link to="/login">
                    <button className="inline-flex items-center gap-2 border border-gray-300 dark:border-white/10 hover:border-gray-400 dark:hover:border-white/20 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white font-semibold px-10 py-4 rounded-2xl transition text-base">
                      Sign In
                    </button>
                  </Link>
                </div>
              )}
              <p className="text-xs text-gray-500 dark:text-gray-600 mt-6">Built for Maharashtra Farmers · Final Year Project · PDEA COE Pune</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      {/* ── Footer ── */}
      <footer id="contact" className="border-t border-gray-200 dark:border-white/5 bg-white dark:bg-gray-950">
        <div className="max-w-7xl mx-auto px-6 py-14">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">

            {/* Brand */}
            <div className="md:col-span-1">
              <div className="flex items-center gap-2.5 font-bold text-lg text-gray-900 dark:text-white mb-3">
                <div className="p-1.5 rounded-lg bg-emerald-500 text-white"><Sprout size={18} /></div>
                Smart<span className="text-emerald-500 dark:text-emerald-400">Agri</span>
              </div>
              ...
              {!loggedIn && (
                <div className="flex gap-3 mt-5">
                  <Link to="/login" className="text-xs border border-gray-300 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white px-4 py-2 rounded-lg hover:border-gray-400 dark:hover:border-white/20 transition">
                    Sign In
                  </Link>
            ...
            <div>
              ...
              <ul className="space-y-2.5 text-sm">
                {[
                  ...
                  <li key={label} className="flex items-center gap-2 text-gray-500 dark:text-gray-500">
          ...
          <div className="border-t border-gray-200 dark:border-white/5 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-gray-400 dark:text-gray-600">© {new Date().getFullYear()} SmartAgri</p>
          </div>
        </div>
      </footer>
    </div>
